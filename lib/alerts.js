...
async function runCheckAlerts() {
  ...
  if (!destinatario) {
    throw new Error(
      'Email destinatario non configurata. Impostala dalla pagina Impostazioni.'
    );
  }
  ...
  const servizi = db
    .prepare(
      `SELECT servizi.*, clienti.nome AS cliente_nome
       ...
       WHERE servizi.stato_rinnovo = 'Attivo'
  ...
  for (const s of servizi) {
    ...
    await sendEmail({ to: destinatario, subject: oggetto, text: testo });
